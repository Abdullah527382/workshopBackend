...
app.post("/createNewRecipe", (request, response) => {
   ...
   db.collection('recipes').add(newRecipe).then((data) => {
...
app.get("/recipe/:recipeID", (request, response) => {
   db.collection('recipes').doc(request.params.recipeID).get().then(data => {
      if (data.exists){
         return response.json({status: "Success", movie: data.data()});
      } else  
      return response.status(404).json({status: "Failed", error: "Recipe not found"});
   }).catch(err => {
